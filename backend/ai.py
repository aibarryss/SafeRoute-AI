...
if not TWOGIS_API_KEY:
    logger.warning("TWOGIS_API_KEY не задан в .env — geocoding и карта работать не будут")
...
async def reverse_geocode(lat: float, lng: float) -> str:
    ...
    try:
        ...
        logger.error("Nominatim fallback error: %s", e)
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            if response.status_code == 200:
                ...
                logger.error("OpenRouter error %d: %s", response.status_code, response.text[:200])
