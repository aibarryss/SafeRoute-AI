...
logger.debug("TWOGIS_API_KEY loaded: %s", 'Yes' if TWOGIS_API_KEY else 'No')
...
class RouteResponse(BaseModel):
    ...
    warnings: List[str] = Field(default_factory=list, description="Предупреждения о маршруте и опасных районах")
    route_buildable: bool = Field(default=True, description="Можно ли построить безопасный маршрут")
...
async def get_config():
    ...
    2GIS ключ нужен для инициализации карты MapGL (требование SDK).

    БЕЗОПАСНОСТЬ: Ключ возвращается ТОЛЬКО если CORS настроен на ваш домен.
    Для продакшена установите ALLOWED_ORIGINS в .env
    """
    return {
...
# asyncio.Lock защищает от race condition при одновременных PATCH/POST обновлениях
_zones: List[Zone] = []
_districts: List[District] = []
_districts_lock = asyncio.Lock()
            ...
            async with httpx.AsyncClient(timeout=8.0) as client:
                ...
                if response.status_code == 200:
                    ...
                    if data.get("code") == "Ok" and data.get("routes"):
                        ...
                        logger.debug("OSRM route cached for %s", cache_key)
                        return route_coords

        except httpx.TimeoutException:
            logger.warning("OSRM timeout on attempt %d/%d", attempt + 1, max_retries)
            if attempt < max_retries - 1:
                await asyncio.sleep(0.5)  # Небольшая пауза перед retry
        except Exception as e:
            logger.warning("OSRM error attempt %d/%d: %s", attempt + 1, max_retries, e)
            if attempt < max_retries - 1:
                await asyncio.sleep(0.3)

    logger.error("OSRM failed after %d attempts", max_retries)
    return None
...
async def update_district(
    ...
    async with _districts_lock:
        ...
        try:
            ...
            logger.info("Обновлён район '%s': danger_level=%d", target.name, request.danger_level)

        except Exception as e:
            logger.error("Ошибка сохранения districts.json: %s", e)
            raise HTTPException(
                status_code=500,
        ...
        if predictor:
            ...
            logger.info("Синхронизирован predictor с обновлёнными районами")
...
async def batch_update_districts(
    ...
    async with _districts_lock:
        ...
        try:
            ...
            logger.info("Массовое обновление: %d районов", len(updated))
...
async def geocode(q: str = ""):
    ...
    try:
        async with httpx.AsyncClient(timeout=8.0) as client:
            ...
            if response.status_code != 200:
                logger.warning("Nominatim HTTP %d: %s", response.status_code, response.text[:200])
                return {"results": []}
...
async def geocode_reverse(lat: float, lng: float):
    ...
    if TWOGIS_API_KEY:
        try:
            ...
            logger.error("2GIS reverse error: %s", e)
    ...
    try:
        ...
        logger.error("Nominatim reverse error: %s", e)
        return {"name": "Моё местоположение", "address": "", "lat": lat, "lng": lng}
...
async def compute_route(request: RouteRequest, req: Request):
    ...
    if osrm_route and len(osrm_route) > 5:
        ...
        logger.info("OSRM: %d точек по реальным дорогам (скорректировано)", len(adjusted))
    else:
        ...
        logger.info("Fallback: %d точек (без OSRM)", len(adjusted))
...
def _generate_heatmap_cpu(predictor, north, south, east, west, grid_size, hour, day):
    """
    CPU-bound функция для генерации heatmap.
    Выполняет все ML предсказания синхронно.
    Вызывается из async handler через run_in_executor.
    """
        ...
        for j in range(grid_size):
            ...
            try:
                ...
                polygon = [
                    ...
                    {"lat": ne_lat, "lng": sw_lng},
                    {"lat": ne_lat, "lng": ne_lng},
                    {"lat": sw_lat, "lng": ne_lng},
                ]
                ...
                logger.error("ML prediction failed for heatmap cell (%.4f, %.4f): %s", center_lat, center_lng, e)
...
async def generate_heatmap(request: HeatmapRequest, req: Request):
    ...
    CPU-bound работа вынесена в отдельный поток через run_in_executor.
    """
    predictor = getattr(req.app.state, 'predictor', None)
    if not predictor:
        raise HTTPException(
    ...
    # CPU-bound ML predictions вынесены в отдельный поток
    loop = asyncio.get_running_loop()
    cells_data = await loop.run_in_executor(
        None,
        _generate_heatmap_cpu,
        predictor, north, south, east, west, request.grid_size, request.hour, request.day,
    )

    return [HeatmapCell(**cell) for cell in cells_data]
